    ...
    <b-card>
      <div class="carrito">
        <b-badge class="vermas" variant="secondary"
          ...
          <ul class="toolip-hover show">
            <li v-for="elem in carro" :key="elem.id">
              Producto: {{ elem.productName }} - Cantidad: {{ elem.cantidadComprar }}
            </li>
          </ul>
        </b-badge>
        <b-button size="sm" variant="secondary">Continuar</b-button>
      </div>
      <b-tabs content-class="mt-3">
        <b-tab title="Juegos" active>
          <div class="row">
            <div v-for="producto in productos" :key="producto.id" class="col-4">
              <tarjeta @quitarProducto="quitarCarro($event)" @producto="anadirCarro($event)" :item="producto" />
            </div>
          </div>
        </b-tab>
      </b-tabs>
